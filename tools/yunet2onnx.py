...
def parse_args():
    ...
    parser.add_argument(
        "--shape", type=int, nargs="+", default=[640, 640], help="input image size"
    )
...
def pytorch2onnx(
    ...
    if verify:
        ...
        onnx_results = session.run(
            None, {session.get_inputs()[0].name: one_img.detach().numpy()}
        )
...
if __name__ == "__main__":
    ...
    if args.output_file is None:
        output_path = (
            "./onnx/" f'{os.path.basename(args.config).rstrip(".py")}' f"_{tag}.onnx"
        )
