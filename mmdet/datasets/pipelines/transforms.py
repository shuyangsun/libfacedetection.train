...
class RandomFlip:
    ...
    def __call__(self, results):
        ...
        if results["flip"]:
            ...
            # flip keypoints
            for key in results.get("keypoints_fields", []):
                results[key] = self.keypoints_flip(
                    results[key], results["img_shape"], results["flip_direction"]
                )
            # flip masks
            for key in results.get("mask_fields", []):
        ...
        while True:
            ...
            for i in range(250):
                ...
                for key in results.get("bbox_fields", []):
                    ...
                    mask_key = self.bbox2mask.get(key)
                    if mask_key in results:
                        results[mask_key] = results[mask_key][mask.nonzero()[0]].crop(
                            patch
                        )
    ...
    def _mosaic_transform(self, results):
        ...
        for i, loc in enumerate(loc_strs):
            ...
            if gt_bboxes_i.shape[0] > 0:
                ...
                if self.use_kps:
                    gt_kpss_i[:, :, 0] = scale_ratio_i * gt_kpss_i[:, :, 0] + padw
                    gt_kpss_i[:, :, 1] = scale_ratio_i * gt_kpss_i[:, :, 1] + padh
        ...
        if len(mosaic_labels) > 0:
            ...
            if self.bbox_clip_border:
                ...
                if self.use_kps:
                    mosaic_kpss[..., 0] = np.clip(
                        mosaic_kpss[..., 0], 0, 2 * self.img_scale[1]
                    )
                    mosaic_kpss[..., 1] = np.clip(
                        mosaic_kpss[..., 1], 0, 2 * self.img_scale[0]
                    )

            # TODO kps skip filter
            if not self.skip_filter:
                valid_inds = self._filter_box_candidates(mosaic_bboxes)
                mosaic_bboxes, mosaic_labels = (
                    mosaic_bboxes[valid_inds],
                    mosaic_labels[valid_inds],
                )
                if self.use_kps:
