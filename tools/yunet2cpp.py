...
def parse_args():
    ...
    parser.add_argument(
        "--output-file", type=str, default="./work_dirs/facedetectcnn-data.cpp"
    )
    parser.add_argument(
        "--no_summary", action="store_true", help="Output the flops and params"
    )
...
class CppConvertor(object):
    ...
    def __init__(self, model):
        ...
        self.cppdata = []
        self.data = """// Auto generated data file
...
"""
    ...
    def convert_param2string(
        ...
        data = {
            ...
            "with_bn": withBNRelu,
            "is_dw": is_depthwise,
            "in_channels": out_channels if is_depthwise else in_channels,
    ...
    def convert_module2string(self, conv, name, module_type):
        if module_type == "Conv_head":
            ...
            self.convert_module2string(
                conv.conv2, name + "_dp", module_type="ConvDPUnit"
            )
        elif module_type == "ConvDPUnit":
            self.cppdata.append(
                self.convert_param2string(conv.conv1, name + "_pw", is_depthwise=False)
            )
            if conv.withBNRelu:
                self.cppdata.append(
                    self.convert_param2string(
            ...
            raise ValueError(
                f"Unsupport module, please comfirm this module: {name} in {self.support_modules} !"
            )
        ...
        for name, m in model.named_children():
            ...
            if m_str in self.support_modules:
                self.module_list.append(
                    {"type": m_str, "name": t_name[2:], "module": m}
                )
            else:
    ...
    def convert(self):
        for m in self.module_list:
            self.convert_module2string(
                conv=m["module"], name=m["name"], module_type=m["type"]
            )
        ...
        for i in range(len(self.cppdata) - 1):
            d = self.cppdata[i]
            struct_str += (
                f"\t{'{'}{d['in_channels']}, {d['out_channels']}, {self.pythonBool2CBool(d['is_dw'])}"
